...
<div id="VerRegistro-modal" class="fixed inset-0 flex items-center justify-center bg-opacity-50 bg-black hidden">
  <div class="p-8 rounded-lg overflow-auto relative" style="background-color: #FBFDFF; height: 800px; width: 805px;">
    ...
    <div class="rounded-full shadow-md items-center justify-center flex text-center m-4" style="background-color: #B4221B; height: 50px;">
      ...
      <button id="edit-doctor-btn" class="w-8 h-8 pt-1 bg-white rounded shadow-md">
    ...
    <div class="p-6 rounded-sm shadow-lg mb-10" style="background-color: #f5f7ff;">
      ...
      <div class="grid gap-6">
        ...
        <!-- Input: Contraseña -->
        <div class="relative mb-4">
          <label for="doctor-password" class="text-gray-700 font-semibold text-sm">Contraseña</label>
          <i class='bx bx-lock absolute left-3 top-3 text-gray-600'></i>
          <input id="doctor-password" class="pl-12 py-2 text-xs bg-[#E6ECF8] rounded-full w-full shadow-md focus:outline-none text-[#3B3636] border-2 border-gray-300 hover:border-[#B4221B] focus:border-[#B4221B]" placeholder="CONTRASEÑA" readonly>
  ...
  fetch(`Solicitudes/obtener_doctor.php?id_doctor=${idDoctor}`)
    ...
    .then(data => {
      if (data) {
        ...
        document.querySelector('#VerRegistro-modal input[placeholder="CONTRASEÑA"]').value = data.Contrasena;
