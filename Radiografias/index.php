                      ...
                      <div class="mb-2">
                        <tr class="bg-sky-100 overflow-hidden " style="border-radius: 50px; box-shadow:0px 5px 6px rgba(3, 64, 179, 0.229); background-color: #e8ecff;">
                            ...
                            <td class="px-4 py-3 text-center" style="border-top-right-radius: 50px; border-bottom-right-radius: 50px;">
                                ...
                                <!-- Botón para ver radiografía -->
                                <button onclick="EliminarRadiografiaModal(<?= $radiografia['id_radiografias'] ?>)" class="bg-transparent border-0 cursor-pointer">
...
<script>
// Función para confirmar y eliminar la radiografía
function EliminarRadiografiaModal(idRadiografia) {
    // Mostrar alerta de confirmación con SweetAlert
    Swal.fire({
        title: '¿Estás seguro?',
        text: "¡Esta acción eliminará permanentemente la radiografía!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            xhr.open('POST', '/ClinicaDentalEsdent/Radiografias/Solicitudes/EliminarRadiografia.php', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            
            // Enviar el ID de la radiografía a eliminar
            xhr.onload = function() {
                if (xhr.status == 200) {
                    var response = JSON.parse(xhr.responseText); // Respuesta del servidor

                    if (response.success) {
                        // Si la eliminación fue exitosa, mostrar mensaje de éxito
                        Swal.fire({
                            title: 'Eliminado!',
                            text: 'La radiografía ha sido eliminada con éxito.',
                            icon: 'success',
                        ...
                        Swal.fire({
                            title: 'Error!',
                            text: 'Hubo un problema al eliminar la radiografía. Intenta de nuevo.',
                            icon: 'error',
                            confirmButtonText: 'Aceptar'
                        });
                    }
            ...
            // Enviar la solicitud con el ID de la radiografía
            xhr.send('idRadiografia=' + idRadiografia);
        }
    });
}
