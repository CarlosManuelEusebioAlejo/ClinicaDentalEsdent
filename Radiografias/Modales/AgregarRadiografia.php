            ...
            <div class="p-6 rounded-sm shadow-md mb-10" style="background-color: #f5f7ff;">
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div class="relative col-span-2">
                        ...
                        <input type="file" name="Foto_Paciente" class="pl-8 py-2 text-xs bg-[#E6ECF8] rounded-full w-full shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-[#3B3636]" onchange="previewImage(event)" />
                    </div>

                    <!-- Previsualización de la imagen -->
                    <div class="relative col-span-2 mt-4" id="imagePreviewContainer">
                        <img id="imagePreview" src="" alt="Previsualización" class="w-full h-auto rounded-md hidden">
                    </div>
                </div>
            </div>
...
function previewImage(event) {
    ...
    reader.onload = function(e) {
        var image = document.getElementById('imagePreview'); // Obtener el contenedor de la imagen
        image.src = e.target.result; // Establecer la fuente de la imagen
        image.classList.remove('hidden'); // Mostrar la imagen
    };
    ...
    xhr.onload = function() {
        if (xhr.status == 200) {
            ...
            if (response.success) {
                ...
                Swal.fire({
                    title: 'Éxito!',
                    text: 'Radiografía agregada con éxito!',
                    icon: 'success',
                ...
                Swal.fire({
                    title: 'Error!',
                    text: 'Hubo un error al agregar la radiografía. Intenta de nuevo.',
                    icon: 'error',
                    confirmButtonText: 'Aceptar'
                });
            }
