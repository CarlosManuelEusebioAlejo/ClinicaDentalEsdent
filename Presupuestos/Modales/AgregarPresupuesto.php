...
<!----------------------- script para que me muestre las comas en los numeros del costo de los precios---------------->
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const inputCosto = document.getElementById('inputCosto');

        inputCosto.addEventListener('input', function () {
            // Eliminar comas del valor actual
            const value = this.value.replace(/,/g, '');

            // Formatear el valor con comas como separadores de miles
            if (!isNaN(value) && value !== '') {
                this.value = Number(value).toLocaleString('en-US');
            } else {
                this.value = ''; // Vaciar si no es válido
            }
        });

        inputCosto.addEventListener('blur', function () {
            // Validar y restaurar valor al perder el enfoque
            const value = this.value.replace(/,/g, '');
            if (isNaN(value) || value === '') {
                this.value = ''; // Restaurar si el valor es inválido
            } else {
                this.value = Number(value).toLocaleString('en-US');
            }
        });

        // Mantener solo números para enviar al servidor
        const form = this.closest('form'); // Asume que el input está en un formulario
        if (form) {
            form.addEventListener('submit', function () {
                inputCosto.value = inputCosto.value.replace(/,/g, '');
            });
        }
    });
</script>
