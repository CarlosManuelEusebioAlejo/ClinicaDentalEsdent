<?php foreach ($pacientes as $pacienteE): ?>
    ...
    <div class="w-full flex flex-col items-center gap-8">
        ...
        <div id="foto-display-<?= htmlspecialchars($pacienteE['idPaciente']) ?>" class="border-2 border-gray-300 w-64 h-64 rounded-md overflow-hidden flex items-center justify-center bg-[#E6ECF8] shadow-lg">
            <?php if (!empty($pacienteE['Foto_paciente'])): ?>
                <!-- Si hay una fotografía cargada previamente -->
                <img id="preview-image-<?= htmlspecialchars($pacienteE['idPaciente']) ?>" src="../<?= str_replace('../', '', $pacienteE['Foto_paciente']) ?>" alt="Fotografía del paciente" class="w-full h-full object-cover">
            <?php else: ?>
                <!-- Si no hay fotografía cargada -->
                <span id="placeholder-<?= htmlspecialchars($pacienteE['idPaciente']) ?>" class="text-gray-500 text-center">Aquí se mostrará la fotografía</span>
            <?php endif; ?>
        </div>

        <!-- Botón para subir o cambiar la fotografía -->
        <div class="flex flex-col items-center">
            <label for="foto-paciente-<?= htmlspecialchars($pacienteE['idPaciente']) ?>" class="cursor-pointer bg-blue-500 text-white py-2 px-4 rounded-full shadow-md hover:bg-blue-600 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                Subir fotografía
            </label>
            <input type="file" id="foto-paciente-<?= htmlspecialchars($pacienteE['idPaciente']) ?>" name="Foto_paciente" accept="image/*" class="hidden" onchange="previewImage(event, <?= htmlspecialchars($pacienteE['idPaciente']) ?>)">
        </div>
    </div>
<script>
// Función para previsualizar la imagen subida
function previewImage(event, pacienteId) {
    const file = event.target.files[0]; // Obtener el archivo de la imagen seleccionada

    // Crear IDs dinámicos basados en el id del paciente
    const previewContainer = document.getElementById('foto-display-' + pacienteId); // Contenedor de la imagen
    const placeholder = document.getElementById('placeholder-' + pacienteId); // Texto de marcador (si no hay imagen)
    const previewImage = document.getElementById('preview-image-' + pacienteId); // Imagen de previsualización existente
    ...
    if (file) {
        ...
        reader.onload = function (e) {
            ...
            // Si ya existe una imagen de previsualización, solo actualizamos el src
            if (previewImage) {
                previewImage.src = e.target.result;
            } else {
                // Si no existe la imagen de previsualización, la creamos
                const newImage = document.createElement('img');
                newImage.id = 'preview-image-' + pacienteId; // ID único para cada imagen
                newImage.src = e.target.result;
                newImage.alt = "Vista previa";
                newImage.className = "w-full h-full object-cover";
                previewContainer.appendChild(newImage); // Añadir la nueva imagen al contenedor
            }
        };
