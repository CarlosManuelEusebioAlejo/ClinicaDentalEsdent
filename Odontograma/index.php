...
<div class="flex flex-col gap-4 p-4">
    <!-- Fila de cuadrados arriba -->
    <div id="contenedor-cuadrados" class="flex gap-4 justify-center"></div>

    <!-- Fila de números abajo -->
    <div id="contenedor-numeros" class="flex gap-4 justify-center"></div>

    <!-- Fila de cuadrados abajo (segunda fila) -->
    <div id="contenedor-cuadrados2" class="flex gap-4 justify-center"></div>

    <!-- Fila de números abajo -->
    <div id="contenedor-numeros2" class="flex gap-4 justify-center"></div>
</div>
...
<script>
    ...
    // Función para crear los cuadrados con números fijos
    function crearCuadradosFijos(filaId, numeros) {
        const contenedorCuadrados = document.getElementById(filaId);
        contenedorCuadrados.innerHTML = ''; // Limpiar contenedor antes de agregar nuevos elementos

        // Crear los cuadrados y asignar los números permanentemente
        numeros.forEach((numero) => {
            const cuadrado = document.createElement('div');
            cuadrado.classList.add('relative', 'w-10', 'h-10', 'border', 'border-black', 'flex', 'items-center', 'justify-center');

            // Crear botones y elementos internos
            cuadrado.innerHTML = `
                <button class="hover-btn hover-top absolute top-0 left-0 w-full h-full clip-path-top" data-pos="Arriba" data-num="${numero}"></button>
                <button class="hover-btn hover-bottom absolute bottom-0 left-0 w-full h-full clip-path-bottom" data-pos="Abajo" data-num="${numero}"></button>
                <button class="hover-btn hover-left absolute top-0 left-0 w-full h-full clip-path-left" data-pos="Izquierda" data-num="${numero}"></button>
                <button class="hover-btn hover-right absolute top-0 right-0 w-full h-full clip-path-right" data-pos="Derecha" data-num="${numero}"></button>
                <button class="hover-btn-center absolute" style="width: 20px; height: 20px; top: 50%; left: 50%; transform: translate(-50%, -50%);" data-pos="Centro" data-num="${numero}"></button>
                ...
                <!-- Número fijo -->
                <span class="absolute text-xs text-center text-black">${numero}</span>
            `;

            // Evento de clic en botones
            const botones = cuadrado.querySelectorAll('button');
            botones.forEach((button) => {
                button.addEventListener('click', (event) => {
                    const posicion = event.target.getAttribute('data-pos');
                    const numeroCuadro = event.target.getAttribute('data-num');
                    mostrarModal(posicion, numeroCuadro);
                });
            });
    ...
    // Función para mostrar números fijos debajo de los cuadrados
    function crearNumerosFijos(filaId, numeros) {
        const contenedorNumeros = document.getElementById(filaId);
        contenedorNumeros.innerHTML = ''; // Limpiar contenedor antes de agregar nuevos elementos

        numeros.forEach((numero) => {
            const numeroBox = document.createElement('div');
            numeroBox.classList.add(
                'w-10', 'h-7', 
            ...
            contenedorNumeros.appendChild(numeroBox);
        });
    }

    // Función para mostrar modal
    function mostrarModal(posicion, numero) {
        document.getElementById('posicion').value = posicion;
        document.getElementById('numeroFila').value = numero;
        document.getElementById('modal').style.display = 'block';
    }

    // Crear las filas de cuadrados y números
    crearCuadradosFijos('contenedor-cuadrados', numerosFila1);
    crearCuadradosFijos('contenedor-cuadrados2', numerosFila2);
    crearNumerosFijos('contenedor-numeros', numerosFila1);
    crearNumerosFijos('contenedor-numeros2', numerosFila2);
</script>
